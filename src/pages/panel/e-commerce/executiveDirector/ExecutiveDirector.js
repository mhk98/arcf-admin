...
} from "../../../../features/chairman/chairman";
...
const ExecutiveDirector = () => {
  ...
  const { data, isLoading, error } = useGetAllChairmanQuery();
  const [createChairman] = useCreateChairmanMutation();
  ...
  const onFormSubmit = async (data) => {
    ...
    try {
      const res = await createChairman(formData);
      if (res) {
  ...
  const [updateChairman] = useUpdateChairmanMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    formData.append("image", image);
    try {
      const response = await updateChairman({ id: updateId, data: formData });
  ...
  const [deleteChairman] = useDeleteChairmanMutation();

  // Function to delete a category
  const handleDeleteSlider = async (id) => {
    try {
      const res = await deleteChairman(id);
      if (res) {
        toast.success(res.data.message);
      }
    } catch (error) {
      ...
      <Content>
        ...
        <Modal isOpen={view.edit} toggle={() => onFormCancel()} className="modal-dialog-centered" size="lg">
          <ModalBody>
            ...
            <div className="p-2">
              <h5 className="title">Update Slider</h5>
              <div className="mt-4">
        ...
        >
          <BlockHead>
            <BlockHeadContent>
              <BlockTitle tag="h5">Add Slider</BlockTitle>
              <BlockDes>
                <p>Add information or update Slider.</p>
              </BlockDes>
            </BlockHeadContent>
          </BlockHead>
          <Block>
