...
} from "../../../../features/eventsDetails/eventsDetails";
...
const EventsDetails = () => {
  ...
  const { data, isLoading, error } = useGetAllEventsDetailsQuery();
  const [createProjectsDetails] = useCreateEventsDetailsMutation();
  ...
  const [updateEventsDetails] = useUpdateEventsDetailsMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    try {
      const response = await updateEventsDetails({ id: updateId, data: formData });
  ...
  const [deleteEventsDetails] = useDeleteEventsDetailsMutation();

  // Function to delete a category
  const handleDeleteBanner = async (id) => {
    try {
      const res = await deleteEventsDetails(id);
      if (res) {
        toast.success(res.data.message);
      }
    } catch (error) {
    ...
    <React.Fragment>
      ...
      <Content>
          ...
          <PreviewAltCard>
            {projects.length > 0 ? (
              ...
              <div className="text-center">
                <span className="text-silent">No events found</span>
              </div>
            )}
          </PreviewAltCard>
        </Block>
        ...
        >
          <BlockHead>
            <BlockHeadContent>
              <BlockTitle tag="h5">Add Events</BlockTitle>
              <BlockDes>
                <p>Add information or update Events.</p>
              </BlockDes>
            </BlockHeadContent>
          </BlockHead>
          <Block>
