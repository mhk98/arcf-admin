...
} from "../../../../features/healthBanner/healthBanner";
...
const HealthBanner = () => {
  ...
  const { data, isLoading, error } = useGetSingleHealthBannerQuery("Health");
  const [createHealthBanner] = useCreateHealthBannerMutation();
  ...
  const onFormSubmit = async (data) => {
    ...
    try {
      const res = await createHealthBanner(formData);
      if (res) {
  ...
  const [updateHealthBanner] = useUpdateHealthBannerMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    try {
      const response = await updateHealthBanner({ id: "Health", data: formData });
  ...
  const [deleteHealthBanner] = useDeleteHealthBannerMutation();

  // Function to delete a category
  const handleDeleteBanner = async () => {
    try {
      const res = await deleteHealthBanner("Health");
      if (res) {
        toast.success(res.data.message);
      }
    } catch (error) {
    ...
    <React.Fragment>
      ...
      <Content>
        ...
        <BlockHead size="sm">
          <BlockBetween>
            <BlockHeadContent>
              <BlockTitle>Project Banner</BlockTitle>
            </BlockHeadContent>
            <BlockHeadContent>
              ...
              <DataTableItem key={projects.Id}>
                <DataTableRow size="sm">
                  <span className="tb-product">
                    <img src={`http://localhost:5000/${projects.image}`} alt="product" className="thumb" />
                  </span>
                </DataTableRow>
          ...
          <PreviewAltCard>
            {projects.length > 0 ? (
              ...
              <div className="text-center">
                <span className="text-silent">No health banner found</span>
              </div>
            )}
          </PreviewAltCard>
        </Block>
