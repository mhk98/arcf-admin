...
const Health = () => {
  ...
  const { data, isLoading, error } = useGetAllHealthQuery();
  const [createHealth] = useCreateHealthMutation();
  ...
  const onFormSubmit = async (data) => {
    ...
    try {
      const res = await createHealth(formData);
      if (res) {
  ...
  const [updateHealth] = useUpdateHealthMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    try {
      const response = await updateHealth({ id: updateId, data: formData });
  ...
  const [deleteHealth] = useDeleteHealthMutation();
