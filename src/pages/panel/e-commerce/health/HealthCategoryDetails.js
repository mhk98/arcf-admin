...
import { useGetAllHealthQuery } from "../../../../features/health/health";
import {
...
const HealthCategoryDetails = () => {
  ...
  const { data, isLoading, error } = useGetAllhealthCategoryDetailsQuery();

  const [createhealthCategoryDetails] = useCreatehealthCategoryDetailsMutation();
  ...
  const onFormSubmit = async (data) => {
    ...
    formData.append("attributeName", quill.root.innerHTML);
    ...
    try {
      const res = await createhealthCategoryDetails({ id: id, data: formData });
      if (res) {
  ...
  const [updatehealthCategoryDetails] = useUpdatehealthCategoryDetailsMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    try {
      const response = await updatehealthCategoryDetails({ id: id, data: formData });
  ...
  const [deletehealthCategoryDetails] = useDeletehealthCategoryDetailsMutation();

  // Function to delete a category
  const handleDeleteBanner = async (id) => {
    try {
      const res = await deletehealthCategoryDetails(id);
      if (res) {
        toast.success(res.data.message);
      }
    } catch (error) {
  ...
  const { data: data1, isLoading: isLoading1, isError: isError1, error: error1 } = useGetAllHealthQuery();
            ...
            {currentItems.length > 0
              ? currentItems.map((item) => (
                  <DataTableItem key={item.Id}>
                    ...
                    <DataTableRow>
                      {" "}
                      <div dangerouslySetInnerHTML={{ __html: item.attributeName }} />
                    </DataTableRow>
                              ...
                              <ul className="link-list-opt no-bdr">
                                ...
                                <li>
                                  <DropdownItem
                                    ...
                                    onClick={(ev) => {
                                      ev.preventDefault();
                                      handleDeleteBanner(item.healthId);
                                    }}
                                  >
