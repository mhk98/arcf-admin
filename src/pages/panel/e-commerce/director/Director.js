...
} from "../../../../features/director/Director";
...
const Director = () => {
  ...
  const { data, isLoading, error } = useGetAllDirectorQuery();
  const [createChairman] = useCreateDirectorMutation();
  ...
  const [updateDirector] = useUpdateDirectorMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    formData.append("image", image);
    try {
      const response = await updateDirector({ id: updateId, data: formData });
    ...
    <React.Fragment>
      ...
      <Content>
        ...
        >
          <BlockHead>
            <BlockHeadContent>
              <BlockTitle tag="h5">Add Director</BlockTitle>
              <BlockDes>
