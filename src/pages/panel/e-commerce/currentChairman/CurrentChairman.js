...
} from "../../../../features/currentChairman/currentChairman";
...
const CurrentChairman = () => {
  ...
  const { data, isLoading, error } = useGetAllCurrentChairmanQuery();
  const [createCurrentChairman] = useCreateCurrentChairmanMutation();
  ...
  const onFormSubmit = async (data) => {
    ...
    try {
      const res = await createCurrentChairman(formData);
      if (res) {
  ...
  const [updateCurrentChairman] = useUpdateCurrentChairmanMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    formData.append("image", image);
    try {
      const response = await updateCurrentChairman({ id: updateId, data: formData });
  ...
  const [deleteCurrentChairman] = useDeleteCurrentChairmanMutation();

  // Function to delete a category
  const handleDeleteSlider = async (id) => {
    try {
      const res = await deleteCurrentChairman(id);
      if (res) {
        toast.success(res.data.message);
      }
    } catch (error) {
      ...
      <Content>
        ...
        <Modal isOpen={view.edit} toggle={() => onFormCancel()} className="modal-dialog-centered" size="lg">
          <ModalBody>
            ...
            <div className="p-2">
              <h5 className="title">Update chairman info</h5>
              <div className="mt-4">
        ...
        >
          <BlockHead>
            <BlockHeadContent>
              <BlockTitle tag="h5">Add Chairman Info</BlockTitle>
              <BlockDes>
...
export default CurrentChairman;
