...
} from "../../../../features/projectSubCategoryDescription/projectSubCategoryDescription";
import { useGetAllProjectsQuery } from "../../../../features/projects/projects";
import Content from "../../../../layout/content/Content";
import Head from "../../../../layout/head/Head";
// import { productData } from "../product/ProductData";

const ProjectSubCategoryDescription = () => {
  ...
  const handleEditorChange = (content) => {
    // Send the content to your Express.js server
  ...
  const { data, isLoading, isError, error } = useGetAllProjectSubCategoryDescriptionQuery();
  ...
  const [createProjectSubCategoryDescription] = useCreateProjectSubCategoryDescriptionMutation();
  ...
  const onFormSubmit = async (data) => {
    ...
    try {
      const res = await createProjectSubCategoryDescription({
        projectId: projectId,
  ...
  const [updateProjectSubCategoryDescription] = useUpdateProjectSubCategoryDescriptionMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    try {
      const response = await updateProjectSubCategoryDescription({
        projectId: projectId,
        subCategoryId: subCategoryId,
        data: formData,
      });
  ...
  const [deleteProjectSubCategoryDescription] = useDeleteProjectSubCategoryDescriptionMutation();

  // Function to delete a category
  const handleDeleteBanner = async (id) => {
    try {
      const res = await deleteProjectSubCategoryDescription(id);
      if (res) {
        toast.success(res.data.message);
      }
    } catch (error) {
          ...
          <PreviewAltCard>
            {categoryDescription.length > 0 ? (
              ...
              <div className="text-center">
                <span className="text-silent">No categoryDescription found</span>
              </div>
            )}
          </PreviewAltCard>
        </Block>
...
export default ProjectSubCategoryDescription;
