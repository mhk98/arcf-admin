...
} from "../../../../features/contactSlider/contactSlider";
...
const ContactSlider = () => {
  ...
  const { data, isLoading, error } = useGetAllContactSliderQuery();
  const [createContactSlider] = useCreateContactSliderMutation();
  ...
  const onFormSubmit = async (data) => {
    ...
    try {
      const res = await createContactSlider(formData);
      if (res) {
  ...
  const [updateContactSlider] = useUpdateContactSliderMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    try {
      const response = await updateContactSlider({ id: updateId, data: formData });
  ...
  const [deleteContactSlider] = useDeleteContactSliderMutation();

  // Function to delete a category
  const handleDeleteSlider = async (id) => {
    try {
      const res = await deleteContactSlider(id);
      if (res) {
        toast.success(res.data.message);
      }
    } catch (error) {
      ...
      <Content>
            ...
            {currentItems.length > 0
              ? currentItems.map((item) => (
                  <DataTableItem key={item.Category_Id}>
                    ...
                    <DataTableRow size="sm">
                      <span className="tb-product">{item.title}</span>
                    </DataTableRow>
          ...
          <PreviewAltCard>
            {slider.length > 0 ? (
              ...
              <div className="text-center">
                <span className="text-silent">No image found</span>
              </div>
            )}
          </PreviewAltCard>
        </Block>
        ...
        <Modal isOpen={view.edit} toggle={() => onFormCancel()} className="modal-dialog-centered" size="lg">
          <ModalBody>
            ...
            <div className="p-2">
              <h5 className="title">Update Image</h5>
              <div className="mt-4">
        ...
        >
          <BlockHead>
            <BlockHeadContent>
              <BlockTitle tag="h5">Add Image</BlockTitle>
              <BlockDes>
                <p>Add information or update Image.</p>
              </BlockDes>
            </BlockHeadContent>
          </BlockHead>
          <Block>
