...
} from "../../../../features/imageGallery/imageGallery";
...
const ImageGallery = () => {
  ...
  const onFormSubmit = async (data) => {
    ...
    try {
      const res = await createImageGallery(formData);
      if (res) {
  ...
  const [updateImageGallery] = useUpdateImageGalleryMutation(); // Use the generated update hook
  ...
  const onEditSubmit = async (data) => {
    ...
    try {
      const response = await updateImageGallery({ id: updateId, data: formData });
  ...
  const handleDeleteSlider = async (id) => {
    try {
      const res = await deleteImageGallery(id);
      if (res) {
        toast.success(res.data.message);
      }
    } catch (error) {
